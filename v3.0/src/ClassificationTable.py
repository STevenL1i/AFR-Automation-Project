...
def get_leaderboard_constructors():
    ...
    for i in range(0, len(group_list)):
        ...
        leaderboard.set_column(0,0, 3)
        leaderboard.set_column(1,1, 21)
        query = "SELECT COUNT(DISTINCT(GP_ENG)) FROM raceCalendar ORDER BY round"
        cursor.execute(query)
        result = cursor.fetchall()
        result = list(result[0])
        race_count = result[0]
        leaderboard.set_column(2, race_count*2 + 1, 4)
        leaderboard.set_column(46, race_count*2 + 2, 9)
        ...
        # flags of each race
        query = "SELECT DISTINCT(GP_ENG) FROM raceCalendar ORDER BY round;"
        cursor.execute(query)
        result = cursor.fetchall()
        tempcursor = 2
        for race in result:
    ...
    for i in range(0, len(group_list)):
        ...
        for driver in result:
            ...
            for r in range(3, 3+race_done):
                ...
                leaderboard.write(row, tempcursor, position, formatter.pointsformat[format])
...
def get_licensepoint():
    ...
    # retirve driverlist from database and write into the table
    query = "SELECT * FROM licensePoint \
